...
then 
    ...
    printf "Usage ./scrape.sh <n>\n"
    printf "    <n> if n is 0, a full run is started, otherwise incremental run of n tasks is started"
    exit 1
fi
...
get_classification() {
    ...
    CLASSIFICATION=$( \
        ...
        | jq -c --arg SEASON_ID "$1" --arg EVENT_ID "$2" --arg CATEGORY_ID "$3" --arg SESSION_ID "$4" \
            '.[] | {season_id: $SEASON_ID, event_id: $EVENT_ID, category_id: $CATEGORY_ID, session_id: $SESSION_ID, name: .name, number: .number, pos: .position, pts: .points}' 2> /dev/null \
        | jq -r '"\(.season_id),\(.event_id),\(.category_id),\(.session_id),\(.name),\(.number),\(.pos),\(.pts)"'
    )

    if [[ $(printf "$CLASSIFICATION" | wc -l) -lt 0 ]]
    then
...
update_tasks() {
    ...
    # COMPLETED
    echo id > $TASK_FILE
    ls $DATA_LAKE | cut -d '.' -f1 >> $TASK_FILE
    update="\
UPDATE tasks
